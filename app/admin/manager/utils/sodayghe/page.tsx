...
import SoDayGheTable from "./components/SoDayGheTable";
...
const Page = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Không thể xóa số dãy ghế");
      }
      ...
      toast.error(
        error instanceof Error ? error.message : "Lỗi khi xóa số dãy ghế"
      );
    } finally {
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      if (isNaN(value) || value < 1) {
        throw new Error("Số dãy ghế không hợp lệ");
      }
      ...
      const url =
        formDialog.mode === "add"
          ? "/api/sodayghe"
          : `/api/sodayghe/${formDialog.item?.id}`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ so_luong_day_ghe: value }),
      });

      if (!response.ok) {
        throw new Error(
          formDialog.mode === "add"
            ? "Lỗi khi thêm số dãy ghế"
            : "Lỗi khi cập nhật số dãy ghế"
        );
      }
      ...
      if (formDialog.mode === "add") {
        setDanhSachSoDayGhe((prev) => [...prev, newItem]);
        setFilteredSoDayGhe((prev) => [...prev, newItem]);
      ...
      toast.error(
        formDialog.mode === "add"
          ? "Có lỗi xảy ra khi thêm số dãy ghế"
          : "Có lỗi xảy ra khi cập nhật số dãy ghế"
      );
    } finally {
  ...
  const openEditDialog = (item: SoDayGhe) => {
    setFormDialog({
      isOpen: true,
      mode: "edit",
      value: item.so_luong_day_ghe.toString(),
      isLoading: false,
      item,
    });
  };
